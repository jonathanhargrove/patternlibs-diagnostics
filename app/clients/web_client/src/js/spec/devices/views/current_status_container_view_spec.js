define(function (require) {
  ...
  describe('CurrentStatusContainerView', function () {
    beforeEach(function () {
      ...
      this.view = new CurrentStatusContainerView({model: this.model, reportCache: {}, session: this.session, system: this.system, customers: this.customersCollection});
    });

    describe('with a device that is opted in', function () {
      it('renders the current status component', function () {
        this.view.render();

        expect(this.view.$el.find('#thermostat-current-status-container').length).toEqual(1);
      });
    });
  });
});
